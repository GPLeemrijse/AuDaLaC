...
fn test_reachability_benchmarks() {
    ...
    let mut paths: Vec<String> = fs::read_dir("tests/benchmarks/reachability")
        .unwrap()
        .map(|p| p.as_ref().unwrap().path().display().to_string())
        .collect();
    ...
    csv.push_str(&format!(
        "n,m,time (ms),problemsize,adl compile time, bin compile time\n"
    ));
...
fn time_binary(file: &str, n: u32) -> (f64, f64, f64) {
    ...
    return (
        ((avg / (n as f64)) * 100.0).round() / 100.0,
        min_time,
        max_time,
    );
}
