...
impl StructManager for CoalescedStructManager<'_> {
    ...
    fn kernels(&self) -> String {
        let step_kernels: Vec<String> = self
            .program
            .structs
            .iter()
            .map(|strct| strct.steps.iter().map(|step| self.step_as_c(strct, step)))
            .flatten()
            .collect();
        let print_kernels: Vec<String> = self
            .program
            .structs
            .iter()
            .map(|strct| self.print_kernel_as_c(strct))
            .collect();
    ...
    fn pre_main(&self) -> String {
        ...
        for (idx, struct_name) in struct_names.iter().enumerate() {
            ...
            to_device.push_str(
                &format! {"\tgm_{struct_name} = ({struct_name}*)host_{struct_name}.to_device();\n"},
            );
        }
    ...
    fn kernel_parameters(&self, _strct: &ADLStruct, step: &Step) -> Vec<String> {
        let mut result: Vec<String> = vec![
            "FPManager* FP".to_string(),
            "inst_size nrof_instances".to_string(),
        ];
...
impl CoalescedStructManager<'_> {
    ...
    fn step_as_c(&self, strct: &ADLStruct, step: &Step) -> String {
        ...
        if constructs.is_empty() {
            ...
            let to_sync = constructs
                .iter()
                .map(|i| {
                    format!(
                        "created_instances |= {}->sync_nrof_instances(host_{});",
                        i.to_lowercase(),
                        i.to_lowercase()
                    )
                })
    ...
    fn print_kernel_as_c(&self, strct: &ADLStruct) -> String {
        ...
        let kernel_parameters =
            format!("{s_name}* {s_name_lwr}{param_indent}inst_size nrof_instances");
        ...
        for stmt in statements {
            ...
            let statement_as_string = match stmt {
                ...
                Assignment(parts_exp, e, _) => {
                    ...
                    let parts_as_c =
                        self.expression_as_c(&Exp::Var(parts.to_vec(), (0, 0)), strct, step);

                    let owner = self.expression_as_c(
                        &Exp::Var(parts[..parts.len() - 1].to_vec(), (0, 0)),
                        strct,
                        step,
                    );
            ...
            Var(parts, _) => {
                ...
                for (t, p) in zip(types[start_idx..].iter(), parts[start_idx..].iter()) {
                    previous_c_expr = format!(
                        "{}->{p}[{previous_c_expr}]{load_suffix}",
                        previous_c_type.to_lowercase()
                    );
                    if t.name().is_some() {
