    ...
    fn schedule_as_c(&self, sched: &Schedule, indent_lvl: usize) -> String {
        ...
        match sched {
            StepCall(step_name, _) => {
                self.step_to_structs
                    .get(step_name)
                    .unwrap()
                    .iter() // iter of structs
                    .map(|s| self.step_call_as_c(s, s.step_by_name(step_name).unwrap(), indent_lvl))
                    .fold("".to_string(), |acc: String, nxt| acc + &nxt)
            }
            Sequential(s1, s2, _) => {
                format!(
                    "{}\n{}",
                    self.schedule_as_c(s1, indent_lvl),
                    self.schedule_as_c(s2, indent_lvl)
                )
            }
            TypedStepCall(struct_name, step_name, _) => {
                ...
                match strct.step_by_name(step_name) {
                    Some(step) => self.step_call_as_c(strct, step, indent_lvl),
                    None => {
