...
impl StepBodyCompiler<'_> {
	...
	pub fn execute_f_name(&self, strct : &String, step : &String) -> String {
		format!("execute_{strct}_{step}")
	}
			...
			&mut self.program
				...
				.map(|strct| {
					strct
						.steps
						.iter()
						.map(|step| self.step_as_c_function(strct, step))
                        .chain(std::iter::once(self.print_as_c_function(strct)))
				})
				.flatten()
				.collect::<Vec<String>>()
		);
	...
	fn step_body_as_c(&self, owner: &ADLStruct, step: &Step) -> String {
		let racing_params = racing_parameters(step, self.program, owner, self.var_exp_type_info);
	...
	fn step_as_c_function(&self, strct: &ADLStruct, step: &Step) -> String {
		let execute_f_name = self.execute_f_name(&strct.name, &step.name);
		
		let step_body = self.step_body_as_c(strct, step);

		self.body_as_device_function(&execute_f_name, &step_body)
	}

	fn print_as_c_function(&self, strct: &ADLStruct) -> String {
		let execute_print_name = self.execute_f_name(&strct.name, &"print".to_string());
		...
		self.body_as_device_function(&execute_print_name, &body)
	}
...
mod tests {
	...
	fn test_local_variable_as_parameter() {
		...
		let received = step_compiler.step_body_as_c(&s1, &step1);
